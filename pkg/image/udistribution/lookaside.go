...
type registryConfiguration struct {
	...
	// The key is a namespace, using fully-expanded Docker reference format or parent namespaces (per udistributionReference.PolicyConfiguration*),
...
func SignatureStorageBaseURL(sys *types.SystemContext, ref types.ImageReference, write bool) (*url.URL, error) {
	dr, ok := ref.(udistributionReference)
	if !ok {
		return nil, errors.Errorf("ref must be a udistributionReference")
	}
...
func (config *registryConfiguration) signatureTopLevel(ref udistributionReference, write bool) string {
