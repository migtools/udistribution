...
type Image struct {
	types.ImageCloser
	src *udistributionImageSource
}

// newImage returns a new Image interface type after setting up
// a client to the registry hosting the given image.
// The caller must call .Close() on the returned Image.
func newImage(ctx context.Context, sys *types.SystemContext, ref udistributionReference) (types.ImageCloser, error) {
...
func GetDigest(ctx context.Context, sys *types.SystemContext, ref types.ImageReference) (digest.Digest, error) {
	dr, ok := ref.(udistributionReference)
	if !ok {
		return "", errors.Errorf("ref must be a udistributionReference")
	}
